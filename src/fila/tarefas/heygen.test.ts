...
import { criarClienteHeygen, criarHeygenBaixar, criarHeygenGerar, escolherUrl, lerChaveHeygen, type ClienteHeygen } from './heygen.js';
...
describe('porTitulo — paginação da video.list', () => {
  // Medido em 2026-08-07: a conta tinha 1400 vídeos e 26 avatares PRONTOS
  // ficaram 21h em `heygen.baixar` sem serem achados. Estavam nas posições
  // 107-169 da lista — logo depois da primeira página de 100, a única que o
  // cliente pedia. O job repetiu 246 vezes e ia expirar em 40h.
  function paginas(...lotes: { video_id: string; video_title: string; status: string }[][]) {
    const vistos: string[] = [];
    const buscar = (async (url: string) => {
      const u = new URL(String(url));
      const t = u.searchParams.get('token');
      vistos.push(t ?? 'pagina-1');
      const i = t ? Number(t) : 0;
      return {
        ok: true,
        json: async () => ({
          data: { videos: lotes[i] ?? [], token: i + 1 < lotes.length ? String(i + 1) : null },
        }),
      } as unknown as Response;
    }) as unknown as typeof fetch;
    return { buscar, vistos };
  }
  const v = (t: string) => ({ video_id: `id-${t}`, video_title: t, status: 'completed' });

  it('acha um título que está na SEGUNDA página', async () => {
    const { buscar } = paginas([v('novo-1')], [v('A39-40mais-v1')]);
    const c = criarClienteHeygen(() => 'k', buscar);
    const r = await c.porTitulo(['A39-40mais-v1']);
    expect(r.get('A39-40mais-v1')).toEqual({ videoId: 'id-A39-40mais-v1', status: 'completed' });
  });

  it('para de paginar assim que achou tudo — não varre a conta inteira', async () => {
    const { buscar, vistos } = paginas([v('achado')], [v('longe')], [v('mais-longe')]);
    const c = criarClienteHeygen(() => 'k', buscar);
    await c.porTitulo(['achado']);
    expect(vistos).toEqual(['pagina-1']);
  });

  it('devolve o que achou quando um dos títulos não existe', async () => {
    const { buscar } = paginas([v('existe')], [v('outro')]);
    const c = criarClienteHeygen(() => 'k', buscar);
    const r = await c.porTitulo(['existe', 'nao-existe']);
    expect([...r.keys()]).toEqual(['existe']);
  });

  it('respeita um teto de páginas em vez de girar sem fim', async () => {
    const lotes = Array.from({ length: 60 }, (_, i) => [v(`v${i}`)]);
    const { buscar, vistos } = paginas(...lotes);
    const c = criarClienteHeygen(() => 'k', buscar);
    await c.porTitulo(['nunca-existiu']);
    expect(vistos.length).toBeLessThanOrEqual(30);
  });
});
