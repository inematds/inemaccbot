...
/** Teto de páginas da `video.list`: 30 x 100 = 3000 vídeos.
 *
 * Existe para o caso do título que NUNCA vai aparecer (erro de digitação no
 * estúdio, fluxo cancelado): sem teto, cada tentativa de um job em espera
 * varreria a conta inteira, e a espera é de 5 em 5 minutos por até 40h. */
export const MAX_PAGINAS_VIDEO_LIST = 30;
...
export function criarClienteHeygen(
  ...
  return {
    ...
    async porTitulo(titulos, sinal) {
      // PAGINA. `limit=100` sozinho não basta: em 2026-08-07 a conta tinha 1400
      // vídeos e 26 avatares PRONTOS ficaram 21h presos em `heygen.baixar`
      // porque estavam nas posições 107-169 — fora da primeira página, a única
      // que este cliente pedia. O job repetiu 246 vezes e ia expirar em 40h.
      // Subir o `limit` foi o remendo da vez anterior; a conta cresceu e o
      // mesmo bug voltou mais fundo. Seguir o `token` é o que não volta.
      const querido = new Set(titulos);
      const saida = new Map<string, VideoHeygen>();
      let token: string | null = null;

      for (let pagina = 0; pagina < MAX_PAGINAS_VIDEO_LIST; pagina++) {
        const url = 'https://api.heygen.com/v1/video.list?limit=100'
        ...
        const dados = (await r.json()) as {
          data?: {
            videos?: { video_id: string; video_title: string; status: string }[];
            token?: string | null;
          };
        };
        ...
        // Achou tudo, ou acabou a lista: parar aqui evita varrer a conta
        // inteira a cada tentativa de um job que fica em espera longa.
        if (saida.size === querido.size) break;
        token = dados.data?.token ?? null;
        if (!token) break;
      }
      return saida;
    },
