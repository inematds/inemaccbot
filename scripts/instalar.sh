...
titulo "3. CTA (ativo externo, não vem em clone nenhum)"
for r in promoavatar promoavatar3; do
  if [ -f "$PROJETOS/$r/cta/cta-9x16.mp4" ]; then ok "$r/cta/cta-9x16.mp4"
  else erro "falta $PROJETOS/$r/cta/cta-9x16.mp4 — copie o CTA oficial (1080x1920, H.264/yuv420p/30fps, AAC 48k)"; fi
done
...
else faca npx playwright install --with-deps chromium; fi
...
if [ -f .env ]; then
  ...
  grep -q '^ALLOWED_CHAT_IDS=.\+' .env || erro "ALLOWED_CHAT_IDS vazio — ponha 0, mande uma mensagem e leia o chat id no log"
...
if [ ${#falta[@]} -eq 0 ]; then
  ...
  Tudo no lugar. Para ligar:
    ...
    node dist/index.js          # confira o boot, Ctrl-C
    systemctl --user enable --now inemaccbot
    loginctl enable-linger "\$USER"

  No chat: /ping, /ajuda, /fila.
FIM
else
